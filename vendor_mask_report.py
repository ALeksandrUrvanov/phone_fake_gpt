"""Отчёт по mask для эталонных заявок (API) — для переписки с вендором."""
...
def mask_bin8(mask: int) -> str:
    """Ровно 8 младших бит — наглядно b0..b7 и bit4/bit7."""
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Отчёт mask по эталонным ID из API")
    ap.add_argument(
        "--vendor",
        default="proposal_id.csv",
        help="CSV с колонкой ID (по умолчанию 10 поддерживаемых эталонных)",
    )
    ap.add_argument("-o", "--out", default="vendor_mask_report.csv", help="Выходной CSV")
    a = ap.parse_args()
    ...
    vendor_ids = sorted(load_vendor_ids(a.vendor))
    if not vendor_ids:
        print(f"В {a.vendor} нет числовых ID")
        sys.exit(1)
